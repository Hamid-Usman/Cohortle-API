...
module.exports = function (app) {
   ...
   */
  app.post("/v1/api/auth/verify-email", [UrlMiddleware], async function (req, res) {
    try {
      ...
      const payload = JwtService.verifyAccessToken(verify_token, process.env.JWT_SECRET);
      if (!payload)
        return res.status(401).json({ error: true, message: "Invalid token or token expired" });
  ...
  app.post("/v1/api/auth/login", [UrlMiddleware], async function (req, res) {
    try {
      ...
      if (!user)
        return res.status(401).json({ error: true, message: "email and password does not match" });

      const isValid = await PasswordService.compareHash(password, user.password);
      if (!isValid)
        return res.status(401).json({ error: true, message: "email and password does not match" });

      const token = JwtService.createAccessToken(
        { user_id: user.id, role: user.role || "unassigned", email: user.email },
        24 * 60 * 60 * 1000,
        process.env.JWT_SECRET
      );
   ...
   */
  app.post("/v1/api/auth/forgot-password", [UrlMiddleware], async function (req, res) {
    try {
      const { email } = req.body;
      const validationResult = await ValidationService.validateObject(
        { email: "required|email" },
      ...
      await MailService.send({
        from: process.env.MAIL_FROM,
        to: email,
        html: MailService.FORGOT_PASSWORD.replace("{{{first_name}}}", user.first_name).replace("{{{link}}}", link),
   ...
   */
  app.post("/v1/api/auth/reset-password", [UrlMiddleware], async function (req, res) {
    try {
